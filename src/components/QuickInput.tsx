...
export default function QuickInput({ onItemCreated }: QuickInputProps) {
  ...
  return (
    <div className="steampunk-border p-3">
      <textarea
        ref={textareaRef}
        value={content}
        ...
        onPaste={handlePaste}
        placeholder={`粘贴或输入内容，按 Ctrl+Enter 保存...（可粘贴≤${MAX_IMAGES}张图片）`}
        className="w-full bg-transparent border-none outline-none resize-none text-sm min-h-[60px] placeholder:text-app-muted/60 text-app-text"
        rows={3}
      />
      ...
      {pendingImages.length > 0 && (
        <div className="mt-2 space-y-1">
          ...
          <div className="text-[10px] text-app-muted/60 text-right">
            {pendingImages.length}/{MAX_IMAGES} 张 · 继续 Ctrl+V 追加
          </div>
        </div>
      )}

      <div className="flex items-center justify-between mt-2 pt-2 border-t border-app-border">
        <div className="flex gap-2 text-app-muted items-center">
          <Image size={14} />
          <button
            type="button"
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="steampunk-button steampunk-button-primary flex items-center gap-1 disabled:opacity-50"
          >
